<?php
/**
 * Course Single - Reviews Tab Content View
 */
...
$summary     = SMLMS_Reviews::get_rating_summary($course_id);
$reviews     = SMLMS_Reviews::get_course_reviews($course_id);
$is_eligible = SMLMS_Reviews::is_user_eligible_to_review($user_id, $course_id);
?>

<div class="smlms-reviews-tab-container">

    <h2 class="smlms-reviews-section-title">Student Reviews</h2>

    <?php if ($summary['total_count'] === 0): ?>
        <p class="smlms-no-reviews-msg">No reviews posted yet.</p>
    <?php else: ?>
        ...
        <!-- Rating Summary Box (Score + Percentage Breakdown) -->
        <div class="smlms-rating-summary-card">
            ...
            <div class="smlms-summary-score-box">
                ...
                <div class="smlms-rating-count-text"><?php echo esc_html($summary['total_count']); ?> Reviews</div>
            </div>
            ...
            ?>
                <div class="smlms-review-item-card" data-rating="<?php echo esc_attr($rev->rating); ?>" data-text="<?php echo esc_attr(strtolower($rev->headline . ' ' . $rev->review_text)); ?>">
                    ...
                    <div class="smlms-review-item-header">
                        <img src="<?php echo esc_url($reviewer_avatar); ?>" class="smlms-reviewer-avatar" alt="Reviewer">
                        
                        <div class="smlms-review-item-meta">
                            ...
                            <div class="smlms-review-author-byline">
                                by <strong><?php echo esc_html($reviewer_name); ?></strong> | <?php echo esc_html($formatted_date); ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="smlms-review-item-footer">
                        <span class="smlms-helpful-label"><span class="smlms-vote-count"><?php echo esc_html($rev->helpful_count); ?></span> people found this helpful</span>
                        <button type="button" class="smlms-btn-vote-helpful" data-review-id="<?php echo esc_attr($rev->id); ?>">Helpful</button>
                    </div>
    ...
    <!-- Write a Review Form or Completion Restriction Box -->
...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '.smlms-btn-vote-helpful', function(e) {
        e.preventDefault();
        var btn = $(this);
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'smlms_vote_helpful',
                review_id: reviewId,
                _wpnonce: nonce
            },
            success: function(res) {
                if (res.success) {
                    btn.closest('.smlms-review-item-footer').find('.smlms-vote-count').text(res.data.new_count);
                }
            }
        });
    });
